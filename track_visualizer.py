...
# color_others=  "yellow"
color_selected_id0 = "red"
color_selected_id = "orange"
color_selected_id1 = "orange"
color_selected_id2 = "orange"
...
class TrackVisualizer(object):
    def __init__(self, config, tracks, tracks_meta, recording_meta):
        ...
        dataset_params_path = Path(self.input_path) / "visualizer_params.json"
        ...
        if "relevant_areas" in self.dataset_params and \
            ...
            logger.warning("Could not find appropriate visualization limits for location {}",
                           recording_meta["locationId"])
        self.ax.axis('off')
    ...
    def _update_figure(self, *args):
        ...
        for track_idx in self.frame_to_track_idxs[self.current_frame]:
            ...
            if self.config["show_bounding_box"] and bounding_box is not None:
                if (track_id==required_id0):
                    bbox = plt.Polygon(bounding_box, True, facecolor=color_selected_id0, **self.bbox_style)
                else:
            ...
            if self.config["show_bounding_box"] and bounding_box is not None:
                if (track_id==required_id2):
                    bbox = plt.Polygon(bounding_box, True, facecolor=color_selected_id2, **self.bbox_style)
                else:
                    bbox = plt.Polygon(bounding_box, True, facecolor=color, **self.bbox_style)
                bbox.set_animated(animate)
            ...
            if (track_id==required_id):
                ...
                bbox = plt.Polygon(bounding_box[3, 0] , True, facecolor="RED", **self.bbox_style)
                bbox.set_animated(animate)
            ...
            if self.config["show_trajectory"]:
                ...
                if center_points.shape[0] > 0:
                    ...
                    if (track_id==required_id0   ):
                        ...
                        plotted_past_line = plt.Polygon(center_points[0:current_index + 1:2], closed=False, color=color_selected_id0 ,
                                                    fill=False, **self.trajectory_style)
            ...
            if annotation_text:
                if (track_id==required_id0   ):
                    ...
                    text_patch = self.ax.text(center_point[0], center_point[1]-2.5, annotation_text,
                                            bbox={"fc": color_selected_id0, **self.text_box_style}, animated=animate,
                                            **self.text_style)
                else:
                    text_patch = self.ax.text(center_point[0], center_point[1]-2.5, annotation_text,
            ...
            if annotation_text:
                if (track_id==required_id2   ):
                    ...
                    text_patch = self.ax.text(center_point[0], center_point[1]-2.5, annotation_text,
                                            bbox={"fc": color_selected_id2, **self.text_box_style}, animated=animate,
                                            **self.text_style)
                else:
                    text_patch = self.ax.text(center_point[0], center_point[1]-2.5, annotation_text,
                                          bbox={"fc": color, **self.text_box_style}, animated=animate,
